...
const apiDescription = [
  ...
  '- **Listen** `typing:start` — `{ channelId, userId }`. Show "X is typing…" in the chat UI.',
  '- **Listen** `typing:stop` — `{ channelId, userId }`. Remove the typing indicator.',
  '',
  '### Mark-read (socket)',
  '',
  ...
  '| `typing:start` | emit / listen | `{ channelId, userId? }` |',
  '| `typing:stop` | emit / listen | `{ channelId, userId? }` |',
