...
const apiDescription = [
  ...
  '',
  'Call `POST /api/v1/users/me/devices` after login, app launch, and every FCM token refresh.',
  '',
  'For iOS, send the Firebase Cloud Messaging registration token as `fcmToken`, not the 64-character APNs token from `getAPNSToken()` / `didRegisterForRemoteNotificationsWithDeviceToken`.',
  '',
  'Good payload: `{ "platform": "IOS", "fcmToken": "<FirebaseMessaging.getToken() value>" }`. Optional PushKit VoIP tokens belong in `apnsVoipToken`, not `token`.',
  '',
  'If using Flutter on iOS, request permission, wait until `FirebaseMessaging.instance.getAPNSToken()` is non-null, then call `FirebaseMessaging.instance.getToken()` and send that FCM token. Also listen to `FirebaseMessaging.instance.onTokenRefresh` and re-register the new token.',
  '',
  '## Avatar / file upload',
  '',
